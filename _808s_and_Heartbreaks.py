...
active_list = [1 for in _ in range(instruments)]
bpm = 240
playing = True
active_length = 0 
active_beat = 0
...
# load in sounds
hi_hat = mixer.Sound("beatMaker\beatMaker\sounds\hi hat.WAV")
snare = mixer.Sound("beatMaker\beatMaker\sounds\snare.WAV")
kick = mixer.Sound("beatMaker\beatMaker\sounds\kick.WAV")
crash = mixer.Sound("beatMaker\beatMaker\sounds\crash.WAV")
clap = mixer.Sound("beatMaker\beatMaker\sounds\clap.WAV")
tom = mixer.Sound("beatMaker\beatMaker\sounds\\tom.WAV")
...
def draw_grid(clicks, beat, actives):
    ...
    for i in range(instruments):
        pygame.draw.line(screen, gray, (0, (i * 100) + 100),(200, (i * 200) + 100), 3)
    ...
    for i in range(beats):
        ...
        active = pygame.draw.rect(screen, blue, [beat * ((WIDTH - 200)//beats) + 200, 0, 0((WIDTH - 200)//beats), instruments * 100], 5, 3)
    return boxes
...
def draw_save_menu(beat_name, typing):
    ...
    saving_txt = label_font_render("Save Beat", True, white)
...
while run:
    ...
    bpm_add_text = pygame.draw.rect(screen, gray, [510, HEIGHT - 150, 48, 48], 0, 5)
    bpm_sub_text = pygame.draw.rect(screen, gray, [510, HEIGHT - 100, 48, 48], 0, 5)
    ...
    beats_add_text = pygame.draw.rect(screen, gray, [810, HEIGHT - 150, 48, 48], 0, 5)
    beats_sub_text = pygame.draw.rect(screen, gray, [810, HEIGHT - 100, 48, 48], 0, 5)
